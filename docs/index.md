# Kinet GPU Core {#mainpage}

Lightweight plugin-based GPU acceleration library for blockchain and ML workloads.

## Overview

Kinet GPU Core provides a unified C API for GPU acceleration with runtime-switchable backends. The library is designed for high-performance cryptographic operations (ZK proofs, FHE, BLS signatures) and tensor computations.
...
- **Unified API** (`kinet/gpu.h`): Application-facing C interface
- **Plugin ABI** (`kinet/gpu/backend_plugin.h`): Stable interface for backends
- **Kernel Loader** (`kinet/gpu/kernel_loader.h`): Generic kernel management
