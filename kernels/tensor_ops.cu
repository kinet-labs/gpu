...
extern "C" __global__
void kinet_add_f32(const float* a, const float* b, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = a[idx] + b[idx];
}

extern "C" __global__
void kinet_sub_f32(const float* a, const float* b, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = a[idx] - b[idx];
}

extern "C" __global__
void kinet_mul_f32(const float* a, const float* b, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = a[idx] * b[idx];
}

extern "C" __global__
void kinet_div_f32(const float* a, const float* b, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = a[idx] / b[idx];
}
...
extern "C" __global__
void kinet_add_f32_vec4(const float4* a, const float4* b, float4* out, uint32_t n4) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n4) {
        float4 va = a[idx];
        float4 vb = b[idx];
...
extern "C" __global__
void kinet_mul_f32_vec4(const float4* a, const float4* b, float4* out, uint32_t n4) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n4) {
        float4 va = a[idx];
        float4 vb = b[idx];
...
extern "C" __global__
void kinet_exp_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = expf(in[idx]);
}

extern "C" __global__
void kinet_log_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = logf(in[idx]);
}

extern "C" __global__
void kinet_sqrt_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = sqrtf(in[idx]);
}

extern "C" __global__
void kinet_neg_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = -in[idx];
}

extern "C" __global__
void kinet_abs_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = fabsf(in[idx]);
}

extern "C" __global__
void kinet_tanh_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = tanhf(in[idx]);
}

extern "C" __global__
void kinet_sigmoid_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = 1.0f / (1.0f + expf(-in[idx]));
}

extern "C" __global__
void kinet_relu_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
extern "C" __global__
void kinet_gelu_f32(const float* in, float* out, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        float x = in[idx];
        float x3 = x * x * x;
...
extern "C" __global__
void kinet_copy_f32(const float* src, float* dst, uint32_t n) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
extern "C" __global__
void kinet_reduce_sum_axis_f32(
    const float* input, float* output,
...
extern "C" __global__
void kinet_reduce_max_axis_f32(
    const float* input, float* output,
    uint32_t outer_size, uint32_t inner_size
) {
    uint32_t gid = blockIdx.x;
...
extern "C" __global__
void kinet_reduce_mean_axis_f32(
    const float* input, float* output,
    uint32_t outer_size, uint32_t inner_size
) {
    uint32_t gid = blockIdx.x;
