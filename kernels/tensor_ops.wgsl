...
fn kinet_add_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
...
@compute @workgroup_size(256)
fn kinet_sub_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
    out[idx] = in_a[idx] - in_b[idx];
}
...
@compute @workgroup_size(256)
fn kinet_mul_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
    out[idx] = in_a[idx] * in_b[idx];
}
...
@compute @workgroup_size(256)
fn kinet_div_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
    out[idx] = in_a[idx] / in_b[idx];
}
...
@compute @workgroup_size(256)
fn kinet_exp_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_log_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_sqrt_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_neg_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
    out[idx] = -in_a[idx];
}
...
@compute @workgroup_size(256)
fn kinet_abs_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_tanh_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_sigmoid_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_relu_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_gelu_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
    let x = in_a[idx];
...
@compute @workgroup_size(256)
fn kinet_copy_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= params.x) { return; }
    out[idx] = in_a[idx];
}
...
fn kinet_matmul_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let row = gid.y;
    let col = gid.x;
    let M = params.x;
    let K = params.y;
...
@compute @workgroup_size(256)
fn kinet_transpose_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    let rows = params.x;
    let cols = params.y;
...
fn kinet_reduce_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let n = params.x;
    let mode = params.w;
...
@compute @workgroup_size(256)
fn kinet_reduce_axis_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let outer = gid.x;
...
@compute @workgroup_size(256)
fn kinet_softmax_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let batch = gid.x;
...
@compute @workgroup_size(256)
fn kinet_log_softmax_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let batch = gid.x;
    if (batch >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_layer_norm_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let batch = gid.x;
    if (batch >= params.x) { return; }
...
@compute @workgroup_size(256)
fn kinet_rms_norm_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let batch = gid.x;
    if (batch >= params.x) { return; }
