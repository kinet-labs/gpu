...
#ifndef KINET_BENCH_COMMON_FIXED_HPP
#define KINET_BENCH_COMMON_FIXED_HPP
...
// Default seed for reproducibility. Override with KINETGPU_BENCH_SEED env var.
...
private:
    BenchRNG() {
        ...
        const char* env_seed = std::getenv("KINETGPU_BENCH_SEED");
        if (env_seed) {
...
inline std::vector<BackendInfo> get_available_backends() {
    ...
    BackendInfo cpu = {KINET_BACKEND_CPU, "CPU", true};
    backends.push_back(cpu);

    BackendInfo metal = {KINET_BACKEND_METAL, "Metal", kinet_backend_available(KINET_BACKEND_METAL)};
    backends.push_back(metal);

    BackendInfo cuda = {KINET_BACKEND_CUDA, "CUDA", kinet_backend_available(KINET_BACKEND_CUDA)};
    backends.push_back(cuda);

    BackendInfo webgpu = {KINET_BACKEND_DAWN, "WebGPU", kinet_backend_available(KINET_BACKEND_DAWN)};
    backends.push_back(webgpu);

    return backends;
}

inline const char* backend_name(KinetBackend backend) {
    switch (backend) {
...
inline bool validate_tensor_modified(KinetGPU* gpu, KinetTensor* input, KinetTensor* output) {
    if (!input || !output) return false;

    int64_t size = kinet_tensor_size(output);
...
inline void print_benchmark_info() {
    ...
    printf("================================================================================\n");
    printf("RNG Seed: %llu (set KINETGPU_BENCH_SEED to override)\n",
...
#endif // KINET_BENCH_COMMON_FIXED_HPP
