...
struct NTTBench {
    ...
    void run(KinetBackend backend) {
        ...
        auto kernel = [&]() {
            KinetError err;
            if (is_inverse) {
                err = kinet_ntt_inverse(gpu, work.data(), n, modulus);
            } else {
                err = kinet_ntt_forward(gpu, work.data(), n, modulus);
            }
            (void)err;
        };
...
struct NTTBatchBench {
    ...
    void run(KinetBackend backend) {
        ...
        auto kernel = [&]() {
            KinetError err = kinet_ntt_batch(gpu, poly_ptrs.data(), batch_size, n, modulus);
            (void)err;
        };
...
struct PolyMulBench {
    ...
    void run(KinetBackend backend) {
        ...
        auto kernel = [&]() {
            // Forward NTT on both
            kinet_ntt_forward(gpu, poly_a.data(), n, modulus);
            kinet_ntt_forward(gpu, poly_b.data(), n, modulus);
            ...
            // Inverse NTT
            kinet_ntt_inverse(gpu, poly_c.data(), n, modulus);
        };
...
struct TFHEBootstrapBench {
    ...
    void run(KinetBackend backend) {
        ...
        auto kernel = [&]() {
            ...
            for (size_t i = 0; i < n; i++) {
                kinet_ntt_forward(gpu, accumulator.data(), N, GOLDILOCKS_PRIME);
                kinet_ntt_forward(gpu, bsk_poly.data(), N, GOLDILOCKS_PRIME);
                ...
                kinet_ntt_inverse(gpu, accumulator.data(), N, GOLDILOCKS_PRIME);
            }
        };
...
int main(int argc, char** argv) {
    printf("=== Kinet GPU FHE Benchmarks ===\n\n");
