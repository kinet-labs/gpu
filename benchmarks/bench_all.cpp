...
struct QuickMatmulBench {
    ...
    void run(KinetBackend backend) {
        result.operation = "matmul";
        result.size = size_str();
        result.backend = backend;
        result.throughput_unit = "GFLOPS";

        KinetGPU* gpu = kinet_gpu_create_with_backend(backend);
        if (!gpu) {
        ...
        KinetTensor* a = kinet_tensor_from_data(gpu, h_a.data(), shape_a, 2, KINET_FLOAT32);
        KinetTensor* b = kinet_tensor_from_data(gpu, h_b.data(), shape_b, 2, KINET_FLOAT32);
        ...
        KinetTensor* c = nullptr;

        std::vector<double> times;
        for (int i = 0; i < 3; i++) {  // Warmup
            if (c) kinet_tensor_destroy(c);
            c = kinet_tensor_matmul(gpu, a, b);
            kinet_gpu_sync(gpu);
        }
...
struct QuickNTTBench {
    ...
    void run(KinetBackend backend) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            kinet_ntt_forward(gpu, work.data(), n, modulus);
            kinet_gpu_sync(gpu);
        }
...
struct QuickElementwiseBench {
    ...
    void run(KinetBackend backend) {
        ...
        KinetTensor* a = kinet_tensor_from_data(gpu, h_a.data(), shape, 1, KINET_FLOAT32);
        KinetTensor* b = kinet_tensor_from_data(gpu, h_b.data(), shape, 1, KINET_FLOAT32);
...
struct QuickReduceBench {
    ...
    void run(KinetBackend backend) {
        ...
        KinetTensor* data = kinet_tensor_from_data(gpu, h_data.data(), shape, 1, KINET_FLOAT32);
        ...
        KinetTensor* out = nullptr;
        int axes[] = {0};

        std::vector<double> times;
        for (int i = 0; i < 3; i++) {
            if (out) kinet_tensor_destroy(out);
            out = kinet_tensor_sum(gpu, data, axes, 1);
            kinet_gpu_sync(gpu);
        }
...
struct QuickMSMBench {
    ...
    void run(KinetBackend backend) {
        ...
        std::vector<double> times;
        for (int i = 0; i < 2; i++) {
            kinet_msm(gpu, scalars.data(), points.data(), msm_result.data(),
                   num_points, KINET_CURVE_BN254);
            kinet_gpu_sync(gpu);
        }
...
struct AllBenchResults {
    ...
    void print_full_table() const {
        std::map<std::string, std::map<KinetBackend, BenchResult>> grouped;
    ...
    void print_summary() const {
        ...
        std::map<KinetBackend, int> success_count;
        std::map<KinetBackend, int> total_count;
        std::map<KinetBackend, double> total_speedup;
        ...
        std::map<std::string, std::map<KinetBackend, double>> throughputs;
        for (const auto& r : results) {
        ...
        KinetBackend all_backends[] = {KINET_BACKEND_CPU, KINET_BACKEND_METAL, KINET_BACKEND_CUDA, KINET_BACKEND_DAWN};
        ...
        for (int i = 0; i < 4; i++) {
            KinetBackend b = all_backends[i];
            int passed = success_count[b];
            int total = total_count[b];

            if (total == 0) {
                printf("%-11s | N/A          | N/A\n", backend_names[i]);
            } else if (b == KINET_BACKEND_CPU) {
...
int main(int argc, char** argv) {
    ...
    printf("================================================================================\n");
    printf("                     Kinet GPU Comprehensive Benchmark Suite\n");
    printf("================================================================================\n");
    ...
    for (const auto& b : backends) {
        if (b.available) {
            KinetGPU* gpu = kinet_gpu_create_with_backend(b.type);
            if (gpu) {
                KinetDeviceInfo info = {};
                kinet_gpu_device_info(gpu, &info);
                printf("  %s:\n", b.name);
