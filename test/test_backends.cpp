...
static BackendResult test_backend(KinetBackend backend, const char* name) {
    BackendResult result = {};
    result.name = name;
    result.available = kinet_backend_available(backend);
    ...
    KinetError err = kinet_gpu_keccak256_batch(gpu, concat_data, gpu_outputs, lengths, 3);
    ...
    // Warm up
    kinet_gpu_keccak256_batch(gpu, bench_data.data(), bench_out.data(),
...
static void test_ecrecover_basic() {
    ...
    KinetEcrecoverOutput addr = {};

    TEST("ecrecover batch dispatch");
    KinetError err = kinet_gpu_ecrecover_batch(gpu, &sig, &addr, 1);
    CHECK(err == KINET_OK, "ecrecover batch should return KINET_OK");

    TEST("ecrecover empty batch");
    err = kinet_gpu_ecrecover_batch(gpu, &sig, &addr, 0);
    CHECK(err == KINET_OK, "empty batch should return KINET_OK");
...
int main() {
    printf("=== Kinet GPU Backend Integration Test ===\n\n");

    // Detect backends
    printf("--- Available Backends ---\n");

    struct { KinetBackend id; const char* name; } backends[] = {
        {KINET_BACKEND_CPU,   "cpu"},
        {KINET_BACKEND_METAL, "metal"},
        {KINET_BACKEND_CUDA,  "cuda"},
        {KINET_BACKEND_DAWN,  "webgpu"},
    };

    for (const auto& b : backends) {
        bool avail = kinet_backend_available(b.id);
        printf("  %-10s %s\n", b.name, avail ? "YES" : "no");
    }
    ...
    for (const auto& b : backends) {
        ...
        if (r.keccak_correct) {
            ...
            KinetGPU* gpu = kinet_gpu_create_with_backend(b.id);
            if (gpu) {
                kinet_gpu_keccak256_batch(gpu, check_data.data(), check_out.data(),
