...
//
// Build:
//   cmake -B build -DKINET_GPU_BUILD_BENCHMARKS=ON -DCMAKE_BUILD_TYPE=Release
//   cmake --build build
...
static void generate_bench_signatures(KinetEcrecoverInput* sigs, size_t n) {
    auto& rng = BenchRNG::instance();

    for (size_t i = 0; i < n; i++) {
        std::memset(&sigs[i], 0, sizeof(KinetEcrecoverInput));
...
struct EcrecoverBench {
    ...
    void run(KinetBackend backend) {
        ...
        std::vector<KinetEcrecoverInput> sigs(num_sigs);
        std::vector<KinetEcrecoverOutput> addrs(num_sigs);

        generate_bench_signatures(sigs.data(), num_sigs);

        auto kernel = [&]() {
            KinetError err = kinet_gpu_ecrecover_batch(
                gpu, sigs.data(), addrs.data(), num_sigs);
            if (err != KINET_OK) {
                result.success = false;
                result.error_msg = kinet_gpu_error(gpu) ? kinet_gpu_error(gpu) : "ecrecover failed";
            }
        };
...
static bool validate_ecrecover(KinetBackend backend) {
    KinetGPU* gpu = kinet_gpu_create_with_backend(backend);
    if (!gpu) return false;
    ...
    KinetEcrecoverInput sig = {};
    sig.v = 0;
    // All-zero r/s should fail validation (r must be > 0)
    KinetEcrecoverOutput out = {};

    KinetError err = kinet_gpu_ecrecover_batch(gpu, &sig, &out, 1);
    kinet_gpu_destroy(gpu);

    if (err != KINET_OK) return false;
...
int main() {
    printf("================================================================================\n");
    printf("       Kinet GPU - secp256k1 ecrecover Benchmark\n");
    printf("================================================================================\n");
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        for (auto& be : backends) {
            if (!be.available) {
                std::string cell = "N/A";
                switch (be.type) {
                    case KINET_BACKEND_CPU:   row.cpu = cell; break;
                    case KINET_BACKEND_METAL: row.metal = cell; break;
                    case KINET_BACKEND_CUDA:  row.cuda = cell; break;
                    case KINET_BACKEND_DAWN:  row.webgpu = cell; break;
                    default: break;
                }
                continue;
            }
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        for (auto& be : backends) {
            ...
            if (be.type == KINET_BACKEND_CPU) {
                cpu_ms = res.stats.median_ms;
            } else if (be.type == KINET_BACKEND_METAL || be.type == KINET_BACKEND_CUDA) {
