...
extern "C" bool kinet_gpu_backend_init(kinet_gpu_backend_desc* out);
...
static BenchResult bench_hash(
    const char* name,
    KinetBackendContext* cpu_ctx, const kinet_gpu_backend_vtbl* cpu_vtbl,
    KinetBackendContext* gpu_ctx, const kinet_gpu_backend_vtbl* gpu_vtbl,
    KinetBackendError (*cpu_fn)(KinetBackendContext*, const uint8_t*, uint8_t*, const size_t*, size_t),
    KinetBackendError (*gpu_fn)(KinetBackendContext*, const uint8_t*, uint8_t*, const size_t*, size_t),
    size_t count)
{
    ...
    // CPU
    double t0 = now_ms();
    KinetBackendError err = cpu_fn(cpu_ctx, inputs.data(), cpu_out.data(), lens.data(), count);
    r.cpu_ms = now_ms() - t0;
    r.cpu_ok = (err == KINET_BACKEND_OK);
...
static BenchResult bench_verify(
    const char* name,
    KinetBackendContext* cpu_ctx,
    KinetBackendContext* gpu_ctx,
    KinetBackendError (*cpu_fn)(KinetBackendContext*, const void*, const void*, const void*, uint32_t*, size_t),
    KinetBackendError (*gpu_fn)(KinetBackendContext*, const void*, const void*, const void*, uint32_t*, size_t),
    ...
    size_t count)
{
    ...
    // CPU
    double t0 = now_ms();
    KinetBackendError err = cpu_fn(cpu_ctx, pubkeys.data(), messages.data(),
    ...
    size_t count)
{
    ...
    // CPU
    double t0 = now_ms();
    KinetBackendError err = cpu_vtbl->op_ecrecover_batch(
    ...
    size_t count)
{
    ...
    // CPU
    double t0 = now_ms();
    KinetBackendError err = cpu_vtbl->op_mlkem_decapsulate_batch(
    ...
    size_t count)
{
    ...
    double t0 = now_ms();
    KinetBackendError err = cpu_vtbl->op_ringtail_partial_sign_batch(
    ...
    size_t count)
{
    ...
    double t0 = now_ms();
    KinetBackendError err = cpu_vtbl->op_cggmp21_partial_sign_batch(
    ...
    size_t count)
{
    ...
    double t0 = now_ms();
    KinetBackendError err = cpu_vtbl->op_frost_partial_verify_batch(
...
{
    printf("================================================================\n");
    printf("Kinet GPU Crypto Benchmark - ALL Operations\n");
    printf("================================================================\n\n");

    // Initialize backends
    kinet_gpu_backend_desc cpu_desc = {};
    kinet_gpu_backend_desc gpu_desc = {};

    bool cpu_avail = cpu_backend_init(&cpu_desc);
    bool gpu_avail = kinet_gpu_backend_init(&gpu_desc);
    ...
    KinetBackendContext* cpu_ctx = cpu_vtbl->create_context(0);
    KinetBackendContext* gpu_ctx = gpu_avail ? gpu_vtbl->create_context(0) : nullptr;
