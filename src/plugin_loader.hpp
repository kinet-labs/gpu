...
#ifndef KINET_GPU_PLUGIN_LOADER_HPP
#define KINET_GPU_PLUGIN_LOADER_HPP
...
public:
    ...
    // Set from environment (KINET_GPU_BACKEND_PATH)
...
#ifdef __APPLE__
    search_paths_.push_back("/usr/local/lib/kinet-gpu");
    search_paths_.push_back("/opt/homebrew/lib/kinet-gpu");
#elif defined(_WIN32)
    search_paths_.push_back("C:\\Program Files\\kinet-gpu\\bin");
#else
    search_paths_.push_back("/usr/lib/kinet-gpu");
    search_paths_.push_back("/usr/local/lib/kinet-gpu");
#endif

    // Environment override
    const char* env_path = std::getenv("KINET_GPU_BACKEND_PATH");
...
inline void PluginLoader::scan_directory(const std::string& dir) {
    // Look for libkinetgpu_backend_*.dylib / .so / .dll
    std::vector<std::string> candidates = {
        "metal", "cuda", "webgpu"
    };

    for (const auto& name : candidates) {
        std::string filename = "libkinetgpu_backend_" + name + DYLIB_EXT;
...
inline bool PluginLoader::load_backend(const std::string& name) {
    ...
    for (const auto& dir : paths) {
        ...
        auto init_fn = reinterpret_cast<kinet_gpu_backend_init_fn>(
            get_symbol(handle, KINET_GPU_BACKEND_INIT_SYMBOL)
        );
        ...
        if (!init_fn(&backend->desc)) {
            ...
            if (backend->desc.abi_version != KINET_GPU_BACKEND_ABI_VERSION) {
                fprintf(stderr, "kinet-gpu: backend %s has ABI version %u, expected %u\n",
                        full_path.c_str(), backend->desc.abi_version, KINET_GPU_BACKEND_ABI_VERSION);
#ifdef _WIN32
                FreeLibrary(handle);
#else
                dlclose(handle);
...
inline bool PluginLoader::load_backend_from_path(const std::string& path) {
    ...
    if (!init_fn(&backend->desc)) {
        ...
        if (backend->desc.abi_version != KINET_GPU_BACKEND_ABI_VERSION) {
            fprintf(stderr, "kinet-gpu: backend %s has ABI version %u, expected %u\n",
                    path.c_str(), backend->desc.abi_version, KINET_GPU_BACKEND_ABI_VERSION);
#ifdef _WIN32
            FreeLibrary(handle);
#else
            dlclose(handle);
...
#endif // KINET_GPU_PLUGIN_LOADER_HPP
