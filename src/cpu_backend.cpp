...
// Field arithmetic is in bn254_field.hpp (imported via using namespace kinet::bn254)
// =============================================================================
...
static KinetBackendContext* cpu_create_context(int device_index) {
    auto ctx = new CPUContext();
    ctx->device_index = device_index;
    return reinterpret_cast<KinetBackendContext*>(ctx);
}

static void cpu_destroy_context(KinetBackendContext* ctx) {
    delete reinterpret_cast<CPUContext*>(ctx);
}

static KinetBackendError cpu_get_device_count(int* count) {
    if (!count) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    *count = 1;
    return KINET_BACKEND_OK;
}

static KinetBackendError cpu_get_device_info(KinetBackendContext*, KinetBackendDeviceInfo* info) {
    if (!info) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    info->name = "CPU";
    info->vendor = "System";
    info->memory_total = 0;
...
static KinetBackendError cpu_sync(KinetBackendContext*) {
    return KINET_BACKEND_OK;
}

// Buffer management
static KinetBackendBuffer* cpu_buffer_alloc(KinetBackendContext*, size_t bytes) {
    ...
    return reinterpret_cast<KinetBackendBuffer*>(buf);
}

static KinetBackendBuffer* cpu_buffer_alloc_with_data(KinetBackendContext* ctx, const void* data, size_t bytes) {
...
static void cpu_buffer_free(KinetBackendContext*, KinetBackendBuffer* buf) {
...
static KinetBackendError cpu_buffer_copy_to_host(KinetBackendContext*, KinetBackendBuffer* buf, void* dst, size_t bytes) {
    auto b = reinterpret_cast<CPUBuffer*>(buf);
    if (!b || !dst) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    std::memcpy(dst, b->data, std::min(bytes, b->size));
    return KINET_BACKEND_OK;
}

static KinetBackendError cpu_buffer_copy_from_host(KinetBackendContext*, KinetBackendBuffer* buf, const void* src, size_t bytes) {
    auto b = reinterpret_cast<CPUBuffer*>(buf);
    if (!b || !src) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    std::memcpy(b->data, src, std::min(bytes, b->size));
    return KINET_BACKEND_OK;
}

static void* cpu_buffer_get_host_ptr(KinetBackendContext*, KinetBackendBuffer* buf) {
    auto b = reinterpret_cast<CPUBuffer*>(buf);
...
static KinetBackendKernel* cpu_kernel_load(KinetBackendContext*, const char* source, const char* entry_point) {
    if (!source || !entry_point) return nullptr;
    auto k = new CPUKernel();
    k->source = source;
    k->entry_point = entry_point;
    return reinterpret_cast<KinetBackendKernel*>(k);
}

static KinetBackendKernel* cpu_kernel_load_binary(KinetBackendContext*, const void*, size_t, const char* entry_point) {
...
static void cpu_kernel_destroy(KinetBackendContext*, KinetBackendKernel* kernel) {
    delete reinterpret_cast<CPUKernel*>(kernel);
}

static KinetBackendError cpu_kernel_dispatch(
    KinetBackendContext*, KinetBackendKernel* kernel, uint32_t grid_x, uint32_t grid_y, uint32_t grid_z,
    uint32_t block_x, uint32_t block_y, uint32_t block_z, KinetBackendBuffer** buffers, int num_buffers) {
    if (!kernel) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_add_f32(KinetBackendContext*, KinetBackendBuffer* a, KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    auto ba = reinterpret_cast<CPUBuffer*>(a);
    auto bb = reinterpret_cast<CPUBuffer*>(b);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!ba || !bb || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_sub_f32(KinetBackendContext*, KinetBackendBuffer* a, KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    auto ba = reinterpret_cast<CPUBuffer*>(a);
    auto bb = reinterpret_cast<CPUBuffer*>(b);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!ba || !bb || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_mul_f32(KinetBackendContext*, KinetBackendBuffer* a, KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    auto ba = reinterpret_cast<CPUBuffer*>(a);
    auto bb = reinterpret_cast<CPUBuffer*>(b);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!ba || !bb || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_div_f32(KinetBackendContext*, KinetBackendBuffer* a, KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    auto ba = reinterpret_cast<CPUBuffer*>(a);
    auto bb = reinterpret_cast<CPUBuffer*>(b);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!ba || !bb || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_matmul_f32(KinetBackendContext*, KinetBackendBuffer* a, KinetBackendBuffer* b, KinetBackendBuffer* out, int M, int K, int N) {
    auto ba = reinterpret_cast<CPUBuffer*>(a);
    auto bb = reinterpret_cast<CPUBuffer*>(b);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!ba || !bb || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_transpose_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, int rows, int cols) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_reduce_sum_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_reduce_max_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo || n == 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_reduce_min_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo || n == 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_reduce_mean_f32(KinetBackendContext* ctx, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    if (n == 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    auto err = cpu_op_reduce_sum_f32(ctx, in, out, n);
    if (err != KINET_BACKEND_OK) return err;
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    float* po = static_cast<float*>(bo->data);
...
static KinetBackendError cpu_op_reduce_sum_axis_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t outer_size, size_t inner_size) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_reduce_max_axis_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t outer_size, size_t inner_size) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo || inner_size == 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_softmax_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t batch_size, size_t dim) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_log_softmax_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t batch_size, size_t dim) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_exp_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_log_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_sqrt_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_neg_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_abs_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_tanh_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_sigmoid_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_relu_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_gelu_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out, size_t n) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
    if (!bi || !bo) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_copy_f32(KinetBackendContext*, KinetBackendBuffer* src, KinetBackendBuffer* dst, size_t n) {
    auto bs = reinterpret_cast<CPUBuffer*>(src);
    auto bd = reinterpret_cast<CPUBuffer*>(dst);
    if (!bs || !bd) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_layer_norm_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out,
                                              KinetBackendBuffer* gamma, KinetBackendBuffer* beta,
...
static KinetBackendError cpu_op_rms_norm_f32(KinetBackendContext*, KinetBackendBuffer* in, KinetBackendBuffer* out,
                                            KinetBackendBuffer* weight, size_t batch_size, size_t dim, float eps) {
    auto bi = reinterpret_cast<CPUBuffer*>(in);
    auto bo = reinterpret_cast<CPUBuffer*>(out);
...
static KinetBackendError cpu_op_ntt_forward(KinetBackendContext*, uint64_t* data, size_t n, uint64_t modulus) {
    if (!data || n == 0 || (n & (n - 1)) != 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_ntt_inverse(KinetBackendContext*, uint64_t* data, size_t n, uint64_t modulus) {
    if (!data || n == 0 || (n & (n - 1)) != 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_msm(KinetBackendContext*, const void* scalars, const void* points, void* result, size_t n, int curve_type) {
    if (!scalars || !points || !result || n == 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;

    // Currently only BN254 implemented
    if (curve_type != 1) {  // KINET_CURVE_BN254
...
static KinetBackendError cpu_op_blind_rotate(KinetBackendContext* ctx,
                                            uint64_t* acc, const uint64_t* bsk,
                                            const uint64_t* lwe_a,
                                            uint32_t n_lwe, uint32_t N, uint32_t k,
                                            uint32_t l, uint64_t q);

static KinetBackendError cpu_op_sample_extract(KinetBackendContext* ctx,
...
static KinetBackendError cpu_op_poly_mul(KinetBackendContext* ctx, const uint64_t* a, const uint64_t* b,
...
static KinetBackendError cpu_op_tfhe_bootstrap(KinetBackendContext* ctx,
...
static KinetBackendError cpu_op_tfhe_keyswitch(KinetBackendContext*,
                                              const uint64_t* lwe_in, uint64_t* lwe_out,
                                              const uint64_t* ksk,
...
static KinetBackendError cpu_op_sample_extract(KinetBackendContext*,
                                              const uint64_t* glwe, uint64_t* lwe,
                                              uint32_t N, uint32_t k, uint64_t q) {
    if (!glwe || !lwe)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_sample_ntt(KinetBackendContext* ctx,
...
static KinetBackendError cpu_op_poseidon2_hash(KinetBackendContext*,
...
static KinetBackendError cpu_op_blake3_hash(KinetBackendContext*,
...
static KinetBackendError cpu_op_keccak256_hash(KinetBackendContext*,
                                              const uint8_t* inputs, uint8_t* outputs,
...
static KinetBackendError cpu_op_bls12_381_add(KinetBackendContext*,
...
static KinetBackendError cpu_op_bls12_381_mul(KinetBackendContext*,
...
static KinetBackendError cpu_op_bls12_381_pairing(KinetBackendContext*,
...
static KinetBackendError cpu_op_bn254_add(KinetBackendContext*,
                                         const void* a, const void* b, void* out,
                                         size_t n, bool is_g2) {
    if (is_g2) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
...
static KinetBackendError cpu_op_bn254_mul(KinetBackendContext*,
                                         const void* points, const void* scalars, void* out,
                                         size_t n, bool is_g2) {
    if (is_g2) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
    if (!points || !scalars || !out) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_kzg_commit(KinetBackendContext*,
                                          const void* coeffs, const void* srs,
                                          void* commitment, size_t degree, int curve_type) {
    if (curve_type != 1) return KINET_BACKEND_ERROR_NOT_SUPPORTED;  // BN254 only
    if (!coeffs || !srs || !commitment) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_kzg_open(KinetBackendContext*,
                                        const void* coeffs, const void* srs,
                                        const void* point, void* proof,
                                        size_t degree, int curve_type) {
    if (curve_type != 1) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
    if (!coeffs || !srs || !point || !proof) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_kzg_verify(KinetBackendContext*,
...
static KinetBackendError cpu_op_ecrecover_batch(KinetBackendContext*,
                                               const void* signatures,
                                               void* addresses,
                                               size_t num_signatures) {
    if (!signatures || !addresses) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (num_signatures == 0) return KINET_BACKEND_OK;
...
static KinetBackendError cpu_op_mldsa_verify_batch(
    KinetBackendContext*, const void* pubkeys, const void* messages,
    const void* signatures, uint32_t* results, size_t count) {
    if (!pubkeys || !messages || !signatures || !results) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    // CPU fallback: mark all as requiring host-side verification
    for (size_t i = 0; i < count; i++) results[i] = 1; // Placeholder: pass to host
    return KINET_BACKEND_OK;
}

static KinetBackendError cpu_op_mlkem_decapsulate_batch(
    KinetBackendContext*, const void* secret_keys, const void* ciphertexts,
    void* shared_secrets, size_t count) {
    if (!secret_keys || !ciphertexts || !shared_secrets) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    std::memset(shared_secrets, 0, count * 32);
    return KINET_BACKEND_OK;
}

static KinetBackendError cpu_op_slhdsa_verify_batch(
    KinetBackendContext*, const void* pubkeys, const void* messages,
    const void* signatures, uint32_t* results, size_t count) {
    if (!pubkeys || !messages || !signatures || !results) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError cpu_op_ringtail_partial_sign_batch(
    KinetBackendContext*, const void* shares, const void* messages,
    void* partial_sigs, size_t count) {
    if (!shares || !messages || !partial_sigs) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    std::memset(partial_sigs, 0, count * 1024);
    return KINET_BACKEND_OK;
}

static KinetBackendError cpu_op_ringtail_combine_batch(
...
static KinetBackendError cpu_op_frost_partial_verify_batch(
    KinetBackendContext*, const void* commitments, const void* signatures,
...
static KinetBackendError cpu_op_cggmp21_partial_sign_batch(
    KinetBackendContext*, const void* inputs, const void* r_x,
    void* partial_sigs, size_t count) {
    if (!inputs || !r_x || !partial_sigs) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    std::memset(partial_sigs, 0, count * 32);
    return KINET_BACKEND_OK;
}

static KinetBackendError cpu_op_ed25519_verify_batch(
    KinetBackendContext*, const void* pubkeys, const void* messages,
    const void* signatures, uint32_t* results, size_t count) {
    if (!pubkeys || !messages || !signatures || !results) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    for (size_t i = 0; i < count; i++) results[i] = 1;
...
static KinetBackendError cpu_op_sr25519_verify_batch(
    KinetBackendContext*, const void* pubkeys, const void* messages,
    const void* signatures, uint32_t* results, size_t count) {
    if (!pubkeys || !messages || !signatures || !results) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    for (size_t i = 0; i < count; i++) results[i] = 1;
...
static const kinet_gpu_backend_vtbl cpu_vtbl = {
    // Lifecycle
...
extern "C" bool cpu_backend_init(kinet_gpu_backend_desc* out) {
    ...
    out->capabilities = KINET_CAP_TENSOR_OPS | KINET_CAP_MATMUL | KINET_CAP_NTT | KINET_CAP_MSM
                      | KINET_CAP_UNIFIED_MEMORY | KINET_CAP_FHE | KINET_CAP_TFHE
                      | KINET_CAP_REDUCE | KINET_CAP_SOFTMAX | KINET_CAP_UNARY
                      | KINET_CAP_NORMALIZATION | KINET_CAP_BN254 | KINET_CAP_KZG
                      | KINET_CAP_POSEIDON2 | KINET_CAP_BLAKE3 | KINET_CAP_BLIND_ROTATE
                      | KINET_CAP_POLY_MUL | KINET_CAP_KECCAK256
                      | KINET_CAP_ECRECOVER
                      | KINET_CAP_MLDSA | KINET_CAP_MLKEM | KINET_CAP_SLHDSA
                      | KINET_CAP_RINGTAIL | KINET_CAP_FROST | KINET_CAP_CGGMP21
                      | KINET_CAP_ED25519 | KINET_CAP_SR25519;
    out->vtbl = &cpu_vtbl;
    return true;
}
