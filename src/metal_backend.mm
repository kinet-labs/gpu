// Metal Backend Plugin - Apple GPU acceleration for kinet-gpu
//
...
// Ecrecover I/O must match gpu.h KinetEcrecoverInput / KinetEcrecoverOutput layout
...
static NSString* find_metal_source(const char* name) {
    const char* env = std::getenv("KINET_GPU_KERNEL_PATH");
    ...
    search_dirs.push_back("/usr/local/share/kinet-gpu/kernels");
    search_dirs.push_back("/opt/homebrew/share/kinet-gpu/kernels");
...
static id<MTLLibrary> compile_metal_source(id<MTLDevice> device, NSString* source) {
    ...
    if (!lib) {
        fprintf(stderr, "kinet-gpu metal: shader compilation failed: %s\n",
...
static id<MTLComputePipelineState> get_pipeline(KinetBackendContext* ctx,
                                                 id<MTLLibrary> lib,
    ...
    if (!func) {
        fprintf(stderr, "kinet-gpu metal: function '%s' not found\n", func_name);
        return nil;
    }
    ...
    if (!pipeline) {
        fprintf(stderr, "kinet-gpu metal: pipeline creation failed for '%s': %s\n",
...
static KinetBackendError dispatch_1d(KinetBackendContext* ctx,
                                    id<MTLComputePipelineState> pipeline,
                                    id<MTLBuffer>* buffers, int num_buffers,
                                    NSUInteger thread_count) {
    @autoreleasepool {
        id<MTLCommandBuffer> cmd = [ctx->queue commandBuffer];
        if (!cmd) return KINET_BACKEND_ERROR_INTERNAL;
        ...
        if ([cmd error]) {
            fprintf(stderr, "kinet-gpu metal: command failed: %s\n",
...
static KinetBackendContext* metal_create_context(int device_index) {
    @autoreleasepool {
...
static void metal_destroy_context(KinetBackendContext* ctx) {
    if (!ctx) return;
    ctx->pipelines.clear();
    ctx->keccak_lib = nil;
...
static KinetBackendError metal_get_device_count(int* count) {
    @autoreleasepool {
...
static KinetBackendError metal_get_device_info(KinetBackendContext* ctx,
...
static KinetBackendError metal_sync(KinetBackendContext* ctx) {
    if (!ctx) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    @autoreleasepool {
        id<MTLCommandBuffer> cmd = [ctx->queue commandBuffer];
        [cmd commit];
        [cmd waitUntilCompleted];
...
static KinetBackendBuffer* metal_buffer_alloc(KinetBackendContext* ctx, size_t bytes) {
...
static KinetBackendBuffer* metal_buffer_alloc_with_data(KinetBackendContext* ctx,
...
static void metal_buffer_free(KinetBackendContext*, KinetBackendBuffer* buf) {
    if (!buf) return;
    buf->mtl = nil;
    delete buf;
}

static KinetBackendError metal_buffer_copy_to_host(KinetBackendContext*,
...
static KinetBackendError metal_buffer_copy_from_host(KinetBackendContext*,
                                                    KinetBackendBuffer* buf,
...
static void* metal_buffer_get_host_ptr(KinetBackendContext*, KinetBackendBuffer* buf) {
...
static KinetBackendKernel* metal_kernel_load(KinetBackendContext* ctx,
                                            const char* source,
    ...
    @autoreleasepool {
        ...
        auto* kernel = new KinetBackendKernel();
...
static KinetBackendKernel* metal_kernel_load_binary(KinetBackendContext* ctx,
...
static void metal_kernel_destroy(KinetBackendContext*, KinetBackendKernel* kernel) {
    if (!kernel) return;
    kernel->pipeline = nil;
    delete kernel;
}
...
static KinetBackendError metal_kernel_dispatch(
    ...
    if (!ctx || !kernel) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    ...
    @autoreleasepool {
        ...
        if ([cmd error]) return KINET_BACKEND_ERROR_DEVICE_LOST;
    }
    return KINET_BACKEND_OK;
}
...
static KinetBackendError metal_binary_op(KinetBackendContext* ctx,
                                        const char* op_name,
                                        KinetBackendBuffer* a, KinetBackendBuffer* b,
                                        KinetBackendBuffer* out, size_t n) {
    if (!ctx || !a || !b || !out || n == 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;

    auto it = ctx->pipelines.find(op_name);
    if (it == ctx->pipelines.end()) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
...
static KinetBackendError metal_unary_op(KinetBackendContext* ctx,
                                       const char* op_name,
                                       KinetBackendBuffer* in, KinetBackendBuffer* out,
                                       size_t n) {
    if (!ctx || !in || !out || n == 0) return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
...
static KinetBackendError metal_op_add_f32(KinetBackendContext* ctx, KinetBackendBuffer* a,
                                         KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    return metal_binary_op(ctx, "add_f32", a, b, out, n);
}
static KinetBackendError metal_op_sub_f32(KinetBackendContext* ctx, KinetBackendBuffer* a,
                                         KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    return metal_binary_op(ctx, "sub_f32", a, b, out, n);
}
static KinetBackendError metal_op_mul_f32(KinetBackendContext* ctx, KinetBackendBuffer* a,
                                         KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    return metal_binary_op(ctx, "mul_f32", a, b, out, n);
}
static KinetBackendError metal_op_div_f32(KinetBackendContext* ctx, KinetBackendBuffer* a,
                                         KinetBackendBuffer* b, KinetBackendBuffer* out, size_t n) {
    return metal_binary_op(ctx, "div_f32", a, b, out, n);
}

// Matmul: naive for now, tiled version later
static KinetBackendError metal_op_matmul_f32(KinetBackendContext* ctx,
                                            KinetBackendBuffer* a, KinetBackendBuffer* b,
                                            KinetBackendBuffer* out,
                                            int M, int K, int N) {
    (void)ctx; (void)a; (void)b; (void)out; (void)M; (void)K; (void)N;
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}

static KinetBackendError metal_op_transpose_f32(KinetBackendContext*, KinetBackendBuffer*,
                                               KinetBackendBuffer*, int, int) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}

// Reductions: not yet implemented on GPU
static KinetBackendError metal_op_reduce_sum_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                KinetBackendBuffer*, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_op_reduce_max_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                KinetBackendBuffer*, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_op_reduce_min_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                KinetBackendBuffer*, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_op_reduce_mean_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                 KinetBackendBuffer*, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_op_reduce_sum_axis_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                     KinetBackendBuffer*, size_t, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_op_reduce_max_axis_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                     KinetBackendBuffer*, size_t, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}

// Softmax
static KinetBackendError metal_op_softmax_f32(KinetBackendContext*, KinetBackendBuffer*,
                                             KinetBackendBuffer*, size_t, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_op_log_softmax_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                 KinetBackendBuffer*, size_t, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}

// Unary ops
static KinetBackendError metal_op_exp_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                         KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "exp_f32", in, out, n);
}
static KinetBackendError metal_op_log_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                         KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "log_f32", in, out, n);
}
static KinetBackendError metal_op_sqrt_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                          KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "sqrt_f32", in, out, n);
}
static KinetBackendError metal_op_neg_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                         KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "neg_f32", in, out, n);
}
static KinetBackendError metal_op_abs_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                         KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "abs_f32", in, out, n);
}
static KinetBackendError metal_op_tanh_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                          KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "tanh_f32", in, out, n);
}
static KinetBackendError metal_op_sigmoid_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                             KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "sigmoid_f32", in, out, n);
}
static KinetBackendError metal_op_relu_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                          KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "relu_f32", in, out, n);
}
static KinetBackendError metal_op_gelu_f32(KinetBackendContext* ctx, KinetBackendBuffer* in,
                                          KinetBackendBuffer* out, size_t n) {
    return metal_unary_op(ctx, "gelu_f32", in, out, n);
}

// Copy
static KinetBackendError metal_op_copy_f32(KinetBackendContext* ctx, KinetBackendBuffer* src,
                                          KinetBackendBuffer* dst, size_t n) {
    return metal_unary_op(ctx, "copy_f32", src, dst, n);
}

// Normalization
static KinetBackendError metal_op_layer_norm_f32(KinetBackendContext*, KinetBackendBuffer*,
                                                KinetBackendBuffer*, KinetBackendBuffer*,
                                                KinetBackendBuffer*, size_t, size_t, float) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_op_rms_norm_f32(KinetBackendContext*, KinetBackendBuffer*,
                                              KinetBackendBuffer*, KinetBackendBuffer*,
                                              size_t, size_t, float) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
...
static KinetBackendError metal_not_supported_ntt(KinetBackendContext*, uint64_t*, size_t, uint64_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_msm(KinetBackendContext*, const void*, const void*,
                                                void*, size_t, int) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_poly_mul(KinetBackendContext*, const uint64_t*,
                                                     const uint64_t*, uint64_t*, size_t, uint64_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_bootstrap(KinetBackendContext*, const uint64_t*,
                                                      uint64_t*, const uint64_t*,
                                                      const uint64_t*, uint32_t, uint32_t,
                                                      uint32_t, uint32_t, uint64_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_keyswitch(KinetBackendContext*, const uint64_t*,
                                                      uint64_t*, const uint64_t*, uint32_t,
                                                      uint32_t, uint32_t, uint32_t, uint64_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_blind_rotate(KinetBackendContext*, uint64_t*,
                                                         const uint64_t*, const uint64_t*,
                                                         uint32_t, uint32_t, uint32_t,
                                                         uint32_t, uint64_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_sample_extract(KinetBackendContext*, const uint64_t*,
                                                           uint64_t*, uint32_t, uint32_t,
                                                           uint64_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_sample_ntt(KinetBackendContext*, uint64_t*, size_t,
                                                       uint64_t, double, uint64_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_poseidon2(KinetBackendContext*, const uint64_t*,
...
static KinetBackendError metal_op_blake3_hash(
    KinetBackendContext* ctx,
    ...
    if (!pipeline) return KINET_BACKEND_ERROR_INTERNAL;
    ...
    @autoreleasepool {
        ...
        if (!desc_buf || !data_buf || !out_buf || !count_buf)
            return KINET_BACKEND_ERROR_OUT_OF_MEMORY;
        ...
        KinetBackendError err = dispatch_1d(ctx, pipeline, bufs, 4, num_hashes);
...
static KinetBackendError metal_not_supported_bls_add(KinetBackendContext*, const void*,
                                                    const void*, void*, size_t, bool) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_bls_mul(KinetBackendContext*, const void*,
                                                    const void*, void*, size_t, bool) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_bls_pairing(KinetBackendContext*, const void*,
                                                        const void*, void*, size_t) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_bn254_add(KinetBackendContext*, const void*,
                                                      const void*, void*, size_t, bool) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_bn254_mul(KinetBackendContext*, const void*,
                                                      const void*, void*, size_t, bool) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_kzg_commit(KinetBackendContext*, const void*,
                                                       const void*, void*, size_t, int) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_kzg_open(KinetBackendContext*, const void*,
                                                     const void*, const void*, void*,
                                                     size_t, int) {
    return KINET_BACKEND_ERROR_NOT_SUPPORTED;
}
static KinetBackendError metal_not_supported_kzg_verify(KinetBackendContext*, const void*,
...
static KinetBackendError metal_op_mldsa_verify_batch(
    KinetBackendContext* ctx,
    const void* pubkeys,
    const void* messages,
    ...
    if (!ctx || !pubkeys || !messages || !signatures || !results)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->mldsa_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
    ...
    @autoreleasepool {
        ...
        KinetBackendError err = dispatch_1d(ctx, pipeline, bufs, 5, count);
        if (err != KINET_BACKEND_OK) return err;
...
static KinetBackendError metal_op_mlkem_decapsulate_batch(
    KinetBackendContext* ctx,
    ...
    if (!ctx || !secret_keys || !ciphertexts || !shared_secrets)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->mlkem_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
    ...
    @autoreleasepool {
        ...
        KinetBackendError err = dispatch_1d(ctx, pipeline, bufs, 4, count);
        if (err != KINET_BACKEND_OK) return err;
...
static KinetBackendError metal_op_slhdsa_verify_batch(
    KinetBackendContext* ctx,
    const void* pubkeys,
    const void* messages,
    const void* signatures,
    ...
    if (!ctx || !pubkeys || !messages || !signatures || !results)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->slhdsa_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
...
static KinetBackendError metal_op_ringtail_partial_sign_batch(
    KinetBackendContext* ctx,
    const void* shares,
    const void* messages,
    void* partial_sigs,
    ...
    if (!ctx || !shares || !messages || !partial_sigs)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->ringtail_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
...
static KinetBackendError metal_op_ringtail_combine_batch(
    KinetBackendContext* ctx,
...
static KinetBackendError metal_op_frost_partial_verify_batch(
    KinetBackendContext* ctx,
    ...
    if (!ctx || !commitments || !signatures || !pubkeys || !challenges || !results)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->frost_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
    ...
    @autoreleasepool {
        ...
        KinetBackendError err = dispatch_1d(ctx, pipeline, bufs, 6, count);
        if (err != KINET_BACKEND_OK) return err;
...
static KinetBackendError metal_op_cggmp21_partial_sign_batch(
    KinetBackendContext* ctx,
    const void* inputs,
    const void* r_x,
    void* partial_sigs,
    ...
    if (!ctx || !inputs || !r_x || !partial_sigs)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->cggmp21_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
...
static KinetBackendError metal_op_ed25519_verify_batch(
    KinetBackendContext* ctx,
    const void* pubkeys,
    const void* messages,
    const void* signatures,
    ...
    if (!ctx || !pubkeys || !messages || !signatures || !results)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->ed25519_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
...
static KinetBackendError metal_op_sr25519_verify_batch(
    KinetBackendContext* ctx,
    const void* pubkeys,
    const void* messages,
    const void* signatures,
    ...
    if (!ctx || !pubkeys || !messages || !signatures || !results)
        return KINET_BACKEND_ERROR_INVALID_ARGUMENT;
    if (count == 0) return KINET_BACKEND_OK;
    if (!ctx->sr25519_lib) return KINET_BACKEND_ERROR_NOT_SUPPORTED;
...
static KinetBackendError metal_op_keccak256_hash(
    KinetBackendContext* ctx,
    const uint8_t* inputs,
    uint8_t* outputs,
    const size_t* input_lens,
    ...
    @autoreleasepool {
        ...
        if (!desc_buf || !data_buf || !out_buf) return KINET_BACKEND_ERROR_OUT_OF_MEMORY;
        ...
        KinetBackendError err = dispatch_1d(ctx, pipeline, bufs, 3, num_inputs);
        if (err != KINET_BACKEND_OK) return err;
...
static KinetBackendError metal_op_ecrecover_batch(
    KinetBackendContext* ctx,
    const void* signatures,
    void* addresses,
    ...
    @autoreleasepool {
        const size_t in_size = num_signatures * 128;   // KinetEcrecoverInput = 128 bytes
        const size_t out_size = num_signatures * 32;    // KinetEcrecoverOutput = 32 bytes
        ...
        if (!in_buf || !out_buf) return KINET_BACKEND_ERROR_OUT_OF_MEMORY;

        // Zero the output buffer
        std::memset([out_buf contents], 0, out_size);

        id<MTLBuffer> bufs[2] = { in_buf, out_buf };
        KinetBackendError err = dispatch_1d(ctx, pipeline, bufs, 2, num_signatures);
        if (err != KINET_BACKEND_OK) return err;
...
static const kinet_gpu_backend_vtbl metal_vtbl = {
    // Lifecycle
...
static bool metal_init(kinet_gpu_backend_desc* out) {
    if (!out) return false;
    ...
    out->capabilities = KINET_CAP_TENSOR_OPS | KINET_CAP_UNIFIED_MEMORY
                       | KINET_CAP_CUSTOM_KERNELS | KINET_CAP_UNARY
                       | KINET_CAP_KECCAK256 | KINET_CAP_ECRECOVER
                       | KINET_CAP_BLAKE3 | KINET_CAP_MLDSA | KINET_CAP_MLKEM
                       | KINET_CAP_SLHDSA | KINET_CAP_RINGTAIL | KINET_CAP_FROST
                       | KINET_CAP_CGGMP21 | KINET_CAP_ED25519 | KINET_CAP_SR25519;
    out->vtbl = &metal_vtbl;
    return true;
}

KINET_GPU_DECLARE_BACKEND(metal_init)
